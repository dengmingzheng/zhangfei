...
class RedisService {
    ...
    public function page() {
        /* 查询redix数据总数 */
        $this->count = Redis::LLEN($this->listKey);
        /* http请求类 */
        $request = app('request');
        /* 接收参数 */
        $input = $request->input();
    ...
    public function count() {
        return Redis::LLEN($this->listKey);
    }
    ...
    public function getList( $from = 0, $to = -1, $idList = [] ) {
        /* 取数据id */
        $idList = $idList ?: Redis::LRANGE($this->listKey, $from, $to);
        /* 根据数据id 读取redis对应的信息 */
        if ( !empty($idList) ) {
            foreach ($idList as $value) {
                /* 判断键是否存在 */
                if ( $this->exists($value) ) {
                    $lists[] = Redis::HGETALL($this->hashKey . $value);
                } else {
    ...
    public function getInfo($value) {
        return Redis::HGETALL($this->hashKey . $value);
    }
    ...
    public function exists($value) {
        return Redis::EXISTS($this->hashKey . $value);
    }
    ...
    public function update( $datas, $field = [] ) {
        ...
        if ( is_array($datas) && !empty($datas) ) {
            ...
            Redis::pipeline(function ($pipe) use ($datas) {
                foreach ($datas as $key => $value) {
                    if (Redis::EXISTS($this->hashKey . $value[$this->idKey])) {
                        $pipe->HMSET($this->hashKey . $value[$this->idKey], $value);
                    }
            ...
            $datas = strval($datas);
            /* redis管道 */
            Redis::pipeline(function ($pipe) use ($field,$datas) {
                foreach ($field as $key => $value) {
                    if (Redis::EXISTS($this->hashKey . $datas)){
    ...
    public function delete( $datas ) {
        if ( is_numeric($datas) && !empty($datas) ) {
            $del = Redis::DEL($this->hashKey . $datas);
            $lrem = Redis::LREM($this->listKey, 0, $datas);
            if ( $del && $lrem ) {
                $result = true;
            } else {
                $result = false;
            ...
            $result = Redis::pipeline(function ($pipe) use ($datas) {
                foreach ($datas as $value) {
    ...
    public function increment( $id, $field, $num = 1 ) {
        if ( Redis::HEXISTS($this->hashKey . $id, $field) ) {
            $value = Redis::HGET($this->hashKey . $id, $field);
            $value = get_numeric($value);
            if ( $value > 0 ) {
                if ( is_int($value) ) {
                    return Redis::HINCRBY($this->hashKey . $id, $field, $num);
                } elseif ( is_float($value) ) {
                    return Redis::HINCRBYFLOAT($this->hashKey . $id, $field, $num);
                }
            }
        }
        return false;
