...
<div class="header_line">
    ...
    <ul class="tab-base">
        ...
        <li><a class="current"><span>添加文章</span></a></li>
    </ul>
</div>


<div class="content-group">
    <form method="post" action="{{url('admin/articles/create')}}" id="form" enctype="multipart/form-data">
        @csrf
        <table class="table tb-type2">
            ...
            <tr class="">
                <td colspan="2" class="required"><label class="validation" for="">所属分类(必选):</label></td>
            </tr>
            <tr class="noborder">
                    ...
                    <select id="ac_id" name="ac_id" class="valid">
                        ...
                        @if(!empty($articleClass))
                            @foreach($articleClass as $val)
                                <option value="{{$val['id']}}" @if(old('ac_id') == $val['id']) selected="selected" @endif>{{$val['title']}}</option>
                                @if(!empty($val['children']))
                                    @foreach($val['children'] as $item)
                                        <option value="{{$item['id']}}" @if(old('ac_id') == $item['id']) selected="selected" @endif>&nbsp;&nbsp;--|{{$item['title']}}</option>
                                    @endforeach
                                @endif
                            @endforeach
                        @endif
            ...
            <tr class="noborder">
                <td colspan="2"><label class="validation">作者(必填):</label></td>
            </tr>
            <tr class="noborder">
                <td class="vatop rowform"><input type="text" value="{{old('author')}}"  name="author" class="txt"></td>
                <td class="vatop tips">{{$errors->first('author')}}</td>
            </tr>

            <tr>
                <td colspan="2" class="required"><label class="validation" for="">排序(必填):</label></td>
            </tr>
            <tr class="noborder">
            ...
            <tr class="noborder">
                <td class="vatop rowform">
                    <input type="radio" value="0" name="is_show" @if(old('is_show') == 0) checked="checked" @endif >草稿 <input type="radio" value="1" name="is_show" @if(old('is_show') == 1) checked="checked" @endif >发布
                </td>
                <td class="vatop tips">{{$errors->first('is_show')}}</td>
            </tr>
